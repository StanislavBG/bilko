# AGENT-001: Rule Architect Protocol
...
1. **Rule Audit** (AGENT-002) - Validates rules against each other
2. **Code Audit** (AGENT-003) - Validates code against rules

## Activation

This protocol is activated when:
- User says "run a rule audit" or "perform rule audit" → Triggers AGENT-002
- User says "run a code audit" or "check code compliance" → Triggers AGENT-003
- User says "act as Rule Architect" → Triggers both audits
- User requests "full audit" → Triggers both audits in sequence

## Audit Types

### Rule Audit (AGENT-002)
...
See AGENT-002 for detailed protocol.

### Code Audit (AGENT-003)
...
See AGENT-003 for detailed protocol.
...
1. Run Rule Audit first (AGENT-002)
2. Fix any critical rule issues
3. Run Code Audit second (AGENT-003)
...
PART 1: RULE AUDIT
[Include full AGENT-002 report]

PART 2: CODE AUDIT
[Include full AGENT-003 report]
